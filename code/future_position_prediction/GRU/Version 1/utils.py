...
def convert_velocity_to_positions(
    ...
    for i in range(seq_len):
        ...
        next_position[:, :2] += predicted_velocity[:, i, :2]  # Update center coordinates
        next_position[:, 2:] *= torch.exp(predicted_velocity[:, i, 2:])  # Update width and height
    ...
    denorm_factor = torch.tensor([image_size[0], image_size[1], image_size[0], image_size[1]], 
                                 device=predicted_positions.device)
    denormalized_positions = predicted_positions * denorm_factor.unsqueeze(0).unsqueeze(0)
