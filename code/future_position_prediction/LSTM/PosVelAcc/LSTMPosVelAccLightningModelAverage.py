...
class LSTMPosVelAccLightningModelAverage(L.LightningModule):
    """
    ...
    def __init__(
        ...
        super(LSTMPosVelAccLightningModelAverage, self).__init__()
    ...
    def forward(
        ...
        position_seq_target: Optional[torch.Tensor] = None,
        velocity_seq_target: Optional[torch.Tensor] = None,
        ...
        """
        # Encode the Position, Velocity, and Acceleration sequences
        _, (encoder_hidden_states_bbox, encoder_cell_states_bbox) = (
            self.position_encoder(bbox_seq)
        )
        _, (encoder_hidden_states_vel, encoder_cell_states_vel) = self.velocity_encoder(
            velocity_seq
        )
        _, (encoder_hidden_states_acc, encoder_cell_states_acc) = (
        ...
        if training_prediction == "recursive":
            ...
            for t in range(self.hparams.output_frames):
                ...
                # Predict with teacher forcing for the next step
                if random.random() < teacher_forcing_ratio:
                    decoder_input_pos = position_seq_target[:, t, :]
                    decoder_input_vel = velocity_seq_target[:, t, :]
                else:
