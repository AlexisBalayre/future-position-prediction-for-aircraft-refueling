...
class LSTMPosVelAccLightningModelConcat(L.LightningModule):
    """
    ...
    def __init__(
        ...
        super(LSTMPosVelAccLightningModelConcat, self).__init__()
    ...
    def combine_hidden_states(
        ...
        """
        Combines the hidden states from position, velocity, and acceleration encoders.

        Args:
            hidden_bbox (torch.Tensor): Hidden states from position encoder.
            hidden_velocity (torch.Tensor): Hidden states from velocity encoder.
            hidden_acceleration (torch.Tensor): Hidden states from acceleration encoder.
    ...
    def combine_cell_states(
        self,
        cell_bbox: torch.Tensor,
        cell_velocity: torch.Tensor,
        cell_acceleration: torch.Tensor,
    ) -> torch.Tensor:
        """
        Combines the cell states from position, velocity, and acceleration encoders.

        Args:
            cell_bbox (torch.Tensor): Cell states from position encoder.
            cell_velocity (torch.Tensor): Cell states from velocity encoder.
            cell_acceleration (torch.Tensor): Cell states from acceleration encoder.

        Returns:
            torch.Tensor: Combined cell state.
        """
    ...
    def forward(
        ...
        hidden_states_pos = hidden_states_vel = self.combine_hidden_states(
            encoder_hidden_states_bbox,
            encoder_hidden_states_vel,
            encoder_hidden_states_acc,
        )
        cell_states_pos = cell_states_vel = self.combine_cell_states(
            encoder_cell_states_bbox, encoder_cell_states_vel, encoder_cell_states_acc
        )
    ...
    def _shared_step(self, batch: Tuple, batch_idx: int, stage: str) -> torch.Tensor:
        ...
        # Predict future velocities and positions
        if stage == "train":
