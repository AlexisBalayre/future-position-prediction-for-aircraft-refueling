...
class LSTMPosVelAccLightningModelClassic(L.LightningModule):
    """
    ...
    def __init__(
        ...
        super(LSTMPosVelAccLightningModelClassic, self).__init__()
    ...
    def forward(
        ...
        if training_prediction == "recursive":
            ...
            for t in range(self.hparams.output_frames):
                ...
                # Use teacher forcing for the next step
                if random.random() < teacher_forcing_ratio:
                    decoder_input_pos = position_seq_target[:, t, :]
                    decoder_input_vel = velocity_seq_target[:, t, :]
    ...
    def _shared_step(
        ...
        Args:
            batch (Tuple[torch.Tensor, ...]): A batch of data containing input sequences and target sequences.
        ...
        """
        _, input_bboxes, input_velocities, _, output_bboxes, output_velocities = batch
