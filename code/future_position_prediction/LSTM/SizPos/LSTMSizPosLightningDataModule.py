...
from .LSTMSizPosLightningDataset import LSTMSizPosLightningDataset


class LSTMSizPosLightningDataModule(L.LightningDataModule):
    """
    A LightningDataModule for managing data loading for the SizPos-LSTM model.
    ...
    def setup(self, stage: str = None, predict_dataset_path: str = None) -> None:
        """
        Set up the datasets for different stages (train, val, test, predict).

        Args:
            stage (str, optional): The stage for which to set up the dataset, either 'train', 'val', 'test', or 'predict'.
            predict_dataset_path (str, optional): Path to the dataset for prediction. Required if stage is 'predict'.
        """
        if stage == "train" or stage is None:
            self.train_dataset = LSTMSizPosLightningDataset(
                self.train_dataset_path,
                input_frames=self.input_frames,
                output_frames=self.output_frames,
                stage="train",
            )
            self.val_dataset = LSTMSizPosLightningDataset(
        ...
        if stage == "test" or stage is None:
            self.test_dataset = LSTMSizPosLightningDataset(
        ...
        if stage == "predict" and predict_dataset_path is not None:
            self.predict_dataset = LSTMSizPosLightningDataset(
