...
class LSTMSizPosLightningDataset(Dataset):
    ...
    for training and evaluating the SizPos-LSTM model.
    ...
    def __getitem__(
        ...
        """
        Retrieves a single sample of input and output sequences, along with computed positions and sizes.

        Args:
            idx (int): Index of the sample to retrieve.

        Returns:
            Tuple: A tuple containing the video ID (str), input positions (torch.Tensor),
                   input sizes (torch.Tensor), output positions (torch.Tensor), and output sizes (torch.Tensor).
        ...
        try:
            ...
            input_positions, input_sizes = self.process_sequence(
                input_bboxes, idx, True
            )
            output_positions, output_sizes = self.process_sequence(
                output_bboxes, idx, False
            )
    ...
    def process_sequence(
        self, bboxes: list, sample_idx: int, is_input: bool
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Process a sequence of bounding boxes into position and size tensors.

        Args:
            bboxes (list): List of bounding boxes.
            sample_idx (int): Index of the sample.
            is_input (bool): Whether the sequence is for input or output.

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Processed positions and sizes as tensors.
        """
        positions = []
        sizes = []
        ...
        return torch.tensor(positions, dtype=torch.float32), torch.tensor(
            sizes, dtype=torch.float32
        )
    ...
    def process_bbox(
        ...
        """
        Process a single bounding box into position and size information.

        Args:
            bbox (list): Bounding box [x, y, w, h].
            seq_idx (int, optional): Index of the sequence. Default is 0.
            sample_idx (int, optional): Index of the sample. Default is 0.
            is_input (bool, optional): Whether the sequence is for input or output. Default is True.

        Returns:
            Tuple[list, list]: Processed position and size information.
        """
