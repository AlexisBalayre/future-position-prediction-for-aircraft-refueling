...
class ConvLSTM(nn.Module):
    ...
    def __init__(
        ...
        if not len(hidden_channels) == len(kernel_size) == num_layers:
    ...
    def _check_kernel_size_consistency(kernel_size):
        if not (isinstance(kernel_size, list) or isinstance(kernel_size, tuple)):
            raise ValueError("`kernel_size` must be list or tuple")
        if not all(isinstance(elem, tuple) for elem in kernel_size):
            raise ValueError("`kernel_size` must be list of tuples")
