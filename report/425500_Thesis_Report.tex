...
There is 3 states of the fueling port in the dataset:
\begin{itemize}
...
The HARD dataset that was provided for this project was not fully annotated.
Therefore, the first step in the data preparation process was to annotate the
dataset. The annotation process involved labelling the fueling port in each
frame of the video sequences. The annotation was performed using the Label
Studio tool, which allows for the creation of bounding boxes around objects of
interest in images and videos.

100 images of each videos were annotated to create a preliminary dataset for training a YOLOv10 model. 
A Label Studio ML backend was then implemented to use the YOLOv10 model and deployed as a Docker container which was used by Label Studio to annotate the remaining images in the dataset.  
This model was then used by a  Label Studio ML backend to annotate the remaining images in the dataset. 
The final dataset consists of 21 video sequences with 1000 annotated images each. 
