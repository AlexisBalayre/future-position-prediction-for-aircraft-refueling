...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/NMS.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/YOLOV10.png}
...
optimizes the model’s architecture across various dimensions. The
...
count. Furthermore, the rank-guided block design minimizes redundancy within
...
also introduces large-kernel convolutions selectively in the deeper stages of
the network to increase the receptive field, allowing the model to capture more
contextual information without a significant increase in computational cost.
Moreover, YOLOv10 incorporates a partial self-attention (PSA) mechanism, which
integrates the benefits of global context modeling while maintaining a
lightweight architecture. This careful balance of efficiency and accuracy
enables YOLOv10 to deliver state-of-the-art performance.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/KernelYOLOv10.png}
...
The Spatio-Temporal Encoder-Decoder (STED) model represents a significant
advancement in multiple object forecasting (MOF) by integrating both visual and
temporal features to predict future object bounding boxes in dynamic
environments. Traditional trajectory forecasting models, which often simplify
objects to single points in space and rely on fixed overhead camera
perspectives, fall short in capturing the rich visual cues and complex motion
patterns necessary for accurate predictions in real-world scenarios. To address
these limitations, the STED model employs a Gated Recurrent Unit (GRU) for
temporal encoding of past bounding box coordinates, effectively capturing the
motion history of objects from 30 previous frames, which corresponds to one
second of video data at 30Hz. Complementing this, the model incorporates a
Convolutional Neural Network (CNN) to extract features from optical flow,
providing detailed insights into both object motion and ego-motion. By
combining these encoded features, the STED model's decoder generates
predictions for the next 60 frames, equivalent to two seconds into the future.

The STED architecture has been rigorously evaluated against several baseline
models, including the Constant Velocity and Constant Scale (CV-CS) model,
Linear Kalman Filter (LKF), Dynamic Trajectory Predictor (DTP), and Future
Person Localization (FPL). The results, presented in Table~\ref{tab:results},
demonstrate the superior performance of the STED model across key metrics. On
the Citywalks dataset, the STED model achieved an Average Displacement Error
(ADE) of 26.0 pixels and a Final Displacement Error (FDE) of 46.9 pixels,
significantly outperforming the baseline models. Additionally, the model showed
enhanced Intersection-over-Union (IoU) metrics, indicating better alignment of
predicted bounding boxes with the ground truth.

\begin{table}[H]
    \centering
    \caption{Performance Comparison of STED Model with Baseline Models on Citywalks and MOT-17 Datasets. Metrics include Average Displacement Error (ADE), Final Displacement Error (FDE), Average Intersection-over-Union (AIOU), and Final Intersection-over-Union (FIOU). The model was evaluated using 30 input frames to predict 60 future frames.}
    \begin{adjustbox}{max width=\textwidth}
        \begin{tabular}{lcccccc}
            \toprule
            \textbf{Model} & \textbf{Dataset}   & \textbf{ADE (pixels)} & \textbf{FDE (pixels)} & \textbf{AIOU (\%)} & \textbf{FIOU (\%)} \\
            \midrule
            CV-CS          & Citywalks          & 31.6                  & 57.6                  & 46.0               & 21.3               \\
            LKF            & Citywalks          & 32.9                  & 59.0                  & 43.9               & 20.1               \\
            DTP            & Citywalks          & 27.3                  & 49.2                  & N/A                & N/A                \\
            FPL            & Citywalks          & 28.6                  & 49.8                  & N/A                & N/A                \\
            \textbf{STED}  & \textbf{Citywalks} & \textbf{26.0}         & \textbf{46.9}         & \textbf{51.8}      & \textbf{27.5}      \\
            \midrule
            CV-CS          & MOT-17             & 58.9                  & 104.7                 & 43.8               & 21.5               \\
            LKF            & MOT-17             & 62.0                  & 110.2                 & 41.6               & 20.1               \\
            DTP            & MOT-17             & 55.2                  & 99.0                  & N/A                & N/A                \\
            FPL            & MOT-17             & 56.9                  & 96.3                  & N/A                & N/A                \\
            \textbf{STED}  & \textbf{MOT-17}    & \textbf{51.8}         & \textbf{91.6}         & \textbf{46.7}      & \textbf{24.4}      \\
            \bottomrule
        \end{tabular}
    \end{adjustbox}
    \label{tab:results}
...
The results highlight the STED model's ability to outperform existing methods,
particularly in terms of lower ADE and FDE, which reflect more accurate
predictions of future object positions. Additionally, the higher AIOU and FIOU
metrics demonstrate that STED achieves better alignment with the actual object
boundaries, further confirming its efficacy in complex, real-world environments
(Figure~\ref{fig:sted}).
